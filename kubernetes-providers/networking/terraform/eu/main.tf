locals {
  wg_port     = 51820
  aws_wg_ip   = "172.16.11.1"
  azure_wg_ip = "172.16.11.2"
  gcp_wg_ip   = "172.16.11.3"
}
...
resource "aws_vpc" "wg" {
  ...
  tags                 = { Name = "${var.name_prefix}-aws-vpc" }
}

resource "aws_internet_gateway" "wg" {
  vpc_id = aws_vpc.wg.id
  tags   = { Name = "${var.name_prefix}-aws-igw" }
}
...
resource "aws_subnet" "wg" {
  ...
  tags                    = { Name = "${var.name_prefix}-aws-subnet" }
}
...
resource "aws_route_table" "wg" {
  ...
  tags = { Name = "${var.name_prefix}-aws-rt" }
}
...
resource "aws_instance" "wg" {
  ...
  tags = { Name = "${var.name_prefix}-aws-wg" }
}

resource "aws_eip" "wg" {
  domain = "vpc"
  tags   = { Name = "${var.name_prefix}-aws-wg-eip" }
}
